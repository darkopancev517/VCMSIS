...
//NOTE: wait until TRNG issue is resolve
#define ECDSA_RANDOM_TEST 0
...
uint32_t ecdsa_std_sign_verify_test_internal(eccp_curve_t *curve, char *curve_name, uint8_t *E, uint32_t EByteLen,
		...
		uint32_t sign_ret_value, char *info)
{
	...
	{
		printf("\r\n %s ECDSA sign %s failure, ret=%08lx", curve_name, inf, ret);
		return 1;
	}
	...
	if(ECDSA_SUCCESS == ret)
	{
		if((NULL != rand_k) && (NULL != signature_r) && (NULL != signature_s))
		{
			...
			{
				printf("\r\n %s ECDSA sign %s error, ret = %02lx", curve_name, inf, ret);
		...
		if(ECDSA_SUCCESS != ret)
		{
			printf("\r\n %s ECDSA verify %s error, ret = %02lx", curve_name, inf, ret);
			return 1;
		}
		...
		{
			printf("\r\n %s ECDSA verify %s failure(invalid E), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		{
			printf("\r\n %s ECDSA verify %s failure(invalid public key 1), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		if(PKE_NOT_ON_CURVE != ret)
		{
			printf("\r\n %s ECDSA verify %s failure(invalid public key 2), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		if(PKE_NOT_ON_CURVE != ret)
		{
			printf("\r\n %s ECDSA verify %s failure(invalid public key 3), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		{
			printf("\r\n %s ECDSA verify %s failure(invalid signature: corrupted r), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		{
			printf("\r\n %s ECDSA verify %s failure(invalid signature: r=0), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		if(ECDSA_VERIFY_FAILED != ret)
		{
			printf("\r\n %s ECDSA verify %s failure(invalid signature: r=n-1), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		{
			printf("\r\n %s ECDSA verify %s failure(invalid signature: r=n), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		if(ECDSA_VERIFY_FAILED != ret)
		{
			printf("\r\n %s ECDSA verify %s failure(invalid signature: corrupted s), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		if(ECDSA_ZERO_ALL != ret)
		{
			printf("\r\n %s ECDSA verify %s failure(invalid signature: s=0), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		if(ECDSA_VERIFY_FAILED != ret)
		{
			printf("\r\n %s ECDSA verify %s failure(invalid signature: s=n-1), ret=%lx", curve_name, inf, ret);
			return 1;
		}
		...
		if(ECDSA_INTEGER_TOO_BIG != ret)
		{
			printf("\r\n %s ECDSA verify %s failure(invalid signature: s=n), ret=%lx", curve_name, inf, ret);
			return 1;
		}
	...
	if(NULL != info)
		printf("\r\n %s ECDSA sign & verify %s success, duration: %lu us", curve_name, inf, duration);
...
uint32_t ecdsa_std_test_brainpoolp160r1(void)
{
	//k:
...
uint32_t ecdsa_std_test_secp192r1(void)
{
	//k:
...
uint32_t ecdsa_std_test_secp224r1(void)
{
	//k:
...
uint32_t ecdsa_std_test_secp384r1(void)
{
	//k:
...
uint32_t ecdsa_std_test_brainpoolp512r1(void)
{
	//k:
...
uint32_t ecdsa_std_test_secp521r1(void)
{
	//k:
...
uint32_t ecdsa_rand_test(const eccp_curve_t *curve, char *curve_name)
{
	uint8_t priKey[66];
	uint8_t pubKey[132];
	uint8_t rand_k[66];
	...
	uint32_t i,j, EByteLen, pByteLen, nByteLen, signatureByteLen;
	uint32_t ret;
	...
	for(i=0;i<ECDSA_SIGN_VERIFY_ROUND1;i++)
	{
		...
		if(PKE_SUCCESS != ret)
		{
			printf("\r\n ECDSA %lu get key error, ret = %02lx", curve->eccp_p_bitLen, ret);
			return 1;
		}

		for(j=0;j<50;j++)
		{
			printf("\r\n ECDSA %lu round1=%lu, round2=%lu", curve->eccp_p_bitLen, i+1, j+1);

			ret = ecdsa_sign((eccp_curve_t *)curve, E, EByteLen, k, priKey, signature);
			if(ECDSA_SUCCESS != ret)
			{
				printf("\r\n ECDSA %lu sign error, ret = %02lx", curve->eccp_p_bitLen, ret);
				print_buf_U8(priKey, pByteLen, "prikey");
			...
			if(ECDSA_SUCCESS != ret)
			{
				printf("\r\n ECDSA %lu verify error, ret = %02lx", curve->eccp_p_bitLen, ret);
				print_buf_U8(priKey, pByteLen, "prikey");
				print_buf_U8(pubKey, pByteLen*2, "pubkey");
				if(k)
					print_buf_U8(k, pByteLen, "k");
...
uint32_t ecdsa_speed_test(eccp_curve_t *curve)
{
	uint8_t priKey[66];
	uint8_t pubKey[132];
	uint8_t E[66];
	uint8_t signature[132];
	uint32_t i, EByteLen, pByteLen, nByteLen, signatureByteLen;
	uint32_t ret;
	...
	//get random key pair
printf("\r\n get key pair begin");fflush(stdout);
for(i=0;i<500;i++)
	ret = eccp_getkey(curve, priKey, pubKey);
printf("\r\n finished");fflush(stdout);
	if(PKE_SUCCESS != ret)
	{
		printf("\r\n ECDSA get key error, ret = %02lx", ret);
...
printf("\r\n finished");fflush(stdout);
	if(ECDSA_SUCCESS != ret)
	{
		printf("\r\n ECDSA verify error, ret = %02lx", ret);
		return 1;
	}
	else
	{
...
uint32_t ecdsa_rng_test(void)
{
	uint8_t priKey[ECCP_MAX_BYTE_LEN];
	uint8_t pubKey[2*ECCP_MAX_BYTE_LEN+1];
	uint8_t signature[2*ECCP_MAX_BYTE_LEN];
	uint8_t E[256]={0};
	uint32_t EByteLen;
	uint32_t i, modulus = 5;
