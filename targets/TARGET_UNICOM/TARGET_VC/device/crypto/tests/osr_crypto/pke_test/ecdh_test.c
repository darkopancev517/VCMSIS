...
uint32_t ecdh_test_internal(const eccp_curve_t *curve, char *curve_name, uint8_t *local_prikey, uint8_t *local_pubkey,
		uint8_t *peer_prikey, uint8_t *peer_pubkey, uint8_t *std_key, uint32_t keyByteLen, KDF_FUNC kdf)
{
	uint8_t local_key[200];
	uint8_t peer_key[200];
	uint8_t tmp[200];
	uint32_t ret;
	...
	if(ECDH_SUCCESS != ret)
	{
		printf("\r\n %s ECDH std test error 1, ret = %02lx", curve_name, ret);
		return 1;
	}

    printf("\r\n %s ECDH standard input-1 test success, duration: %lu us", curve_name, duration);
	...
	{
		printf("\r\n %s ECDH std test error 2, ret = %02lx", curve_name, ret);
		return 1;
	}

    printf("\r\n %s ECDH standard input-2 test success, duration: %lu us", curve_name, duration);
	...
	if(ECDH_SUCCESS == ret)
	{
		printf("\r\n %s ECDH corner case test error 1, ret = %02lx", curve_name, ret);
		return 1;
	}

    printf("\r\n %s ECDH corner case (peer pubkey all zero) test success, duration: %lu us", curve_name, duration);
	...
	if(ECDH_SUCCESS == ret)
	{
		printf("\r\n %s ECDH corner case test error 2, ret = %02lx", curve_name, ret);
		return 1;
	}

    printf("\r\n %s ECDH corner case (peer pubkey corrupted) test success, duration: %lu us", curve_name, duration);
	...
	if(ECDH_SUCCESS == ret)
	{
		printf("\r\n %s ECDH corner case test error 3, ret = %02lx", curve_name, ret);
		return 1;
	}

    printf("\r\n %s ECDH corner case (local_prikey is 0) test success, duration: %lu us", curve_name, duration);

	//corner case: local_prikey is n
	reverse_byte_array((uint8_t *)curve->eccp_n, (uint8_t *)tmp, GET_BYTE_LEN(curve->eccp_n_bitLen));
	...
	if(ECDH_SUCCESS == ret)
	{
		printf("\r\n %s ECDH corner case test error 4, ret = %02lx", curve_name, ret);
		return 1;
	}

    printf("\r\n %s ECDH corner case (local_prikey is n) test success, duration: %lu us", curve_name, duration);
	...
	if(ECDH_SUCCESS == ret)
	{
		printf("\r\n %s ECDH corner case test error 5, ret = %02lx", curve_name, ret);
		return 1;
	}

    printf("\r\n %s ECDH corner case (local_prikey is n+1) test success, duration: %lu us", curve_name, duration);
...
uint32_t ecdh_std_test_brainpoolp160r1(void)
{
	uint8_t local_prikey[20]={
		0x62,0x31,0xB2,0xC9,0xF5,0x85,0x18,0x56,0x01,0xF5,0x9F,0x27,0xC0,0xC0,0x09,0x2C,
		0x18,0xA8,0xC5,0x86,};
	uint8_t local_pubkey[40]={
...
uint32_t ecdh_std_test_secp224r1(void)
{
	uint8_t local_prikey[28]={
...
uint32_t ecdh_std_test_secp256r1(void)
{
	uint8_t local_prikey[32]={
...
uint32_t ecdh_std_test_secp384r1(void)
{
	uint8_t local_prikey[48]={
...
uint32_t ecdh_std_test_brainpoolp512r1(void)
{
	uint8_t local_prikey[64]={
...
uint32_t ecdh_std_test_secp521r1(void)
{
	uint8_t local_prikey[66]={
...
uint32_t ecdh_rand_test(const eccp_curve_t *curve, char *curve_name)
{
	uint8_t local_prikey[66];
	uint8_t local_pubkey[132];
	uint8_t peer_prikey[66];
	...
	uint8_t peer_key[200];
	uint32_t i,j;
	uint32_t keyByteLen, trueKeyByteLen;
	KDF_FUNC kdf=NULL;
	uint32_t ret;
	...
	for(i=0;i<ECDH_SIGN_VERIFY_ROUND1;i++)
	{
		...
		if(PKE_SUCCESS != ret)
		{
			printf("\r\n %s ECDH A get key error, ret=%lu", curve_name, ret);
			return 1;
		}
		...
		if(PKE_SUCCESS != ret)
		{
			printf("\r\n %s ECDH B get key error, ret=%lu", curve_name, ret);
			return 1;
		}

		for(j=0;j<50;j++)
		{
			printf("\r\n %s ECDH round1=%lu, round2=%lu", curve_name, i+1, j+1);
			...
			if(ECDH_SUCCESS != ret)
			{
				printf("\r\n %s ECDH test error 1, ret = %02lx", curve_name, ret);
				return 1;
			}
			...
			{
				printf("\r\n %s ECDH test error 2, ret = %02lx", curve_name, ret);
				return 1;
			}
		}
	}
...
uint32_t ecdh_speed_test(const eccp_curve_t *curve)
{
	uint8_t local_prikey[66];
	uint8_t local_pubkey[132];
	uint8_t peer_prikey[66];
	...
	for(i=0;i<10;i++)
	{
		...
		if(PKE_SUCCESS != ret)
		{
			printf("\r\n ECDH %lu A get key error, ret=%lu", curve->eccp_p_bitLen, ret);
			return 1;
		}
		...
		if(PKE_SUCCESS != ret)
		{
			printf("\r\n ECDH %lu B get key error, ret=%lu", curve->eccp_p_bitLen, ret);
			return 1;
		}
	}
...
printf("\r\n finished");fflush(stdout);
	if(ECDH_SUCCESS != ret)
	{
		printf("\r\n ECDH %lu test error 1, ret = %02lx", curve->eccp_p_bitLen, ret);
		return 1;
	}
	...
	{
		printf("\r\n ECDH %lu test error 2, ret = %02lx", curve->eccp_p_bitLen, ret);
		return 1;
	}
...
#if 0 // NOTE: skipped rand test until TRNG_IP has been fixed
...
#endif // NOTE: skipped rand test until TRNG_IP has been fixed
