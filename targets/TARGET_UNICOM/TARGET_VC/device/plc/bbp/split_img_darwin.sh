...
if [ "$MACHINE" = "x86_64" ]; then 
  TOOLCHAIN=../tools/toolchain/gcc-arm-none-eabi-4_9-2015q3/bin
...
$2 ~ /(\.data$|\.rodata$|^\.text$)/ {
  ...
  #print cmd
  system(cmd)
  cmd = sprintf("printf \"%%08x\" `gstat -c \"%%s\" %s.xz` > compressed_size_%s", bin_file, zip_loc)
  #print cmd
  system(cmd)
}
$2 ~ /\.PhoenixBbpFwVersion$/ {
  cmd = objcopy " -I elf32-little -O binary -j .PhoenixBbpFwVersion " img_file " magpie_bbp_fw_ver.bin"
  #print cmd
  system(cmd)
}
...
gsed -i "s/REPLACE_z_data1/$COMPRESS_SIZE/g" magpie_bbp_sec_map.txt
COMPRESS_SIZE=`cat compressed_size_z_data0`
gsed -i "s/REPLACE_z_data0/$COMPRESS_SIZE/g" magpie_bbp_sec_map.txt
COMPRESS_SIZE=`cat compressed_size_z_iram`
gsed -i "s/REPLACE_z_iram/$COMPRESS_SIZE/g" magpie_bbp_sec_map.txt
