...
void vcana_set_gppll_200mhz(void)
{
  ...
  uint32_t temp = VC_ANA->RG_ABBBG_CTRL;
  temp &= ~VC_ANA_RG_ABBBG_CTRL_EN_Msk;
  temp &= ~VC_ANA_RG_ABBBG_CTRL_TRIMVOL_Msk;
  temp |= (0x3UL << VC_ANA_RG_ABBBG_CTRL_TRIMVOL_Pos);
  temp |= (1UL << VC_ANA_RG_ABBBG_CTRL_EN_Pos);
  VC_ANA->RG_ABBBG_CTRL = temp;

  // Set DDSM test = 1
  temp = VC_ANA->RG_SOC_GPPLL_CTRL;
  temp &= ~VC_ANA_RG_SOC_GPPLL_CTRL_DDSM_TEST_Msk;
  temp |= (1UL << VC_ANA_RG_SOC_GPPLL_CTRL_DDSM_TEST_Pos);
  VC_ANA->RG_SOC_GPPLL_CTRL = temp;

  temp = VC_ANA->RG_SOC_GPPLL_TESTIN;
  temp &= ~VC_ANA_RG_SOC_GPPLL_TESTIN_Msk;
  temp |= (0x600UL << VC_ANA_RG_SOC_GPPLL_TESTIN_Pos);
  VC_ANA->RG_SOC_GPPLL_TESTIN = temp;

  temp = VC_ANA->RG_SOC_GPPLL_METER_CTRL;
  temp &= ~VC_ANA_RG_SOC_GPPLL_METER_CTRL_PLL_WAIT_TIME_Msk;
  temp &= ~VC_ANA_RG_SOC_GPPLL_METER_CTRL_FM_MANUALSEL_Msk;
  temp &= ~VC_ANA_RG_SOC_GPPLL_METER_CTRL_FNPLL_IBAND_RST_Msk;
  temp |= (0x3fUL << VC_ANA_RG_SOC_GPPLL_METER_CTRL_PLL_WAIT_TIME_Pos);
  temp |= (1UL << VC_ANA_RG_SOC_GPPLL_METER_CTRL_FM_MANUALSEL_Pos);
  temp |= (1UL << VC_ANA_RG_SOC_GPPLL_METER_CTRL_FNPLL_IBAND_RST_Pos);
  VC_ANA->RG_SOC_GPPLL_METER_CTRL = temp;

  temp = VC_ANA->RG_GPPLL_CTRL1;
  temp &= ~VC_ANA_RG_GPPLL_CTRL1_POSDIVSEL_Msk;
  temp &= ~VC_ANA_RG_GPPLL_CTRL1_PI_RSTB_Msk;
  temp &= ~VC_ANA_RG_GPPLL_CTRL1_EN_Msk;
  temp |= (0x2UL << VC_ANA_RG_GPPLL_CTRL1_POSDIVSEL_Pos);
  temp |= (1UL << VC_ANA_RG_GPPLL_CTRL1_EN_Pos);
  VC_ANA->RG_GPPLL_CTRL1 = temp;

  temp = VC_ANA->RG_SOC_GPPLL_METER_CTRL;
  temp &= ~VC_ANA_RG_SOC_GPPLL_METER_CTRL_FNPLL_IBAND_RST_Msk;
  temp &= ~VC_ANA_RG_SOC_GPPLL_METER_CTRL_FM_EN_Msk;
  temp |= (1UL << VC_ANA_RG_SOC_GPPLL_METER_CTRL_FM_EN_Pos);
  VC_ANA->RG_SOC_GPPLL_METER_CTRL = temp;

  temp = VC_ANA->RG_SOC_GPPLL_DDSMIN;
  temp &= ~VC_ANA_RG_SOC_GPPLL_DDSMIN_Msk;
  temp |= (0x01800000UL << VC_ANA_RG_SOC_GPPLL_DDSMIN_Pos);
  VC_ANA->RG_SOC_GPPLL_DDSMIN = temp;

  temp = VC_ANA->RG_SOC_GPPLL_OFFSETIN;
  temp &= ~VC_ANA_RG_SOC_GPPLL_OFFSETIN_Msk;
  VC_ANA->RG_SOC_GPPLL_OFFSETIN = temp;

  temp = VC_ANA->RG_SOC_GPPLL_CTRL;
  temp &= ~VC_ANA_RG_SOC_GPPLL_CTRL_DDSM_TEST_Msk;
  temp &= ~VC_ANA_RG_SOC_GPPLL_CTRL_DDSM_EN_Msk;
  temp |= (1UL << VC_ANA_RG_SOC_GPPLL_CTRL_DDSM_EN_Pos);
  VC_ANA->RG_SOC_GPPLL_CTRL = temp;

  temp = VC_MISC2->CLKSEL;
  temp &= ~VC_MISC2_CLKSEL_CLKSEL_Msk;
  temp |= VC_MISC2_CLKSEL_CLKSEL_GPLL;
...
void vcana_set_bbpll_300mhz(void)
{
  ...
  temp = VC_ANA->RG_SOC_FNPLL_CTRL;
  temp &= ~VC_ANA_RG_SOC_FNPLL_DDSM_TEST_Msk;
  temp |= (1UL << VC_ANA_RG_SOC_FNPLL_DDSM_TEST_Pos);
  VC_ANA->RG_SOC_FNPLL_CTRL = temp;

  temp = VC_ANA->RG_SOC_FNPLL_TESTIN;
  temp &= ~VC_ANA_RG_SOC_FNPLL_TESTIN_Msk;
  temp |= (0x600UL << VC_ANA_RG_SOC_FNPLL_TESTIN_Pos);
  VC_ANA->RG_SOC_FNPLL_TESTIN = temp;

  temp = VC_ANA->RG_SOC_FNPLL_METER_CTRL;
  temp &= ~VC_ANA_RG_SOC_FNPLL_METER_CTRL_PLL_WAIT_TIME_Msk;
  temp &= ~VC_ANA_RG_SOC_FNPLL_METER_CTRL_FNPLL_IBAND_RST_Msk;
  temp |= (0x3FUL << VC_ANA_RG_SOC_FNPLL_METER_CTRL_PLL_WAIT_TIME_Pos);
  temp |= (1UL << VC_ANA_RG_SOC_FNPLL_METER_CTRL_FNPLL_IBAND_RST_Pos);
  VC_ANA->RG_SOC_FNPLL_METER_CTRL = temp;

  temp = VC_ANA->RG_BBPLL_CTRL1;
  temp &= ~VC_ANA_RG_BBPLL_CTRL1_ADCCK_EN_Msk;
  temp &= ~VC_ANA_RG_BBPLL_CTRL1_DACCK_EN_Msk;
  temp &= ~VC_ANA_RG_BBPLL_CTRL1_DSPCK_EN_Msk;
  temp &= ~VC_ANA_RG_BBPLL_CTRL1_WBADC_EN_Msk;
  temp &= ~VC_ANA_RG_BBPLL_CTRL1_IR_Msk;
  temp &= ~VC_ANA_RG_BBPLL_CTRL1_PC_Msk;
  temp &= ~VC_ANA_RG_BBPLL_CTRL1_EN_Msk;
  temp |= (1UL << VC_ANA_RG_BBPLL_CTRL1_ADCCK_EN_Pos);
  temp |= (1UL << VC_ANA_RG_BBPLL_CTRL1_DSPCK_EN_Pos);
  temp |= (1UL << VC_ANA_RG_BBPLL_CTRL1_WBADC_EN_Pos);
  temp |= (2UL << VC_ANA_RG_BBPLL_CTRL1_IR_Pos);
  temp |= (0x10UL << VC_ANA_RG_BBPLL_CTRL1_PC_Pos);
  temp |= (1UL << VC_ANA_RG_BBPLL_CTRL1_EN_Pos);
  VC_ANA->RG_BBPLL_CTRL1 = temp;

  temp = VC_ANA->RG_SOC_FNPLL_METER_CTRL;
  temp &= ~VC_ANA_RG_SOC_FNPLL_METER_CTRL_FNPLL_IBAND_RST_Msk;
  temp &= ~VC_ANA_RG_SOC_FNPLL_METER_CTRL_FM_EN_Msk;
  temp |= (1UL << VC_ANA_RG_SOC_FNPLL_METER_CTRL_FM_EN_Pos);
  VC_ANA->RG_SOC_FNPLL_METER_CTRL = temp;

  temp = VC_ANA->RG_SOC_FNPLL_DDSMIN;
  temp &= ~VC_ANA_RG_SOC_FNPLL_DDSMIN_Msk;
  temp |= (0x01800000UL << VC_ANA_RG_SOC_FNPLL_DDSMIN_Pos);
  VC_ANA->RG_SOC_FNPLL_DDSMIN = temp;

  temp = VC_ANA->RG_SOC_FNPLL_OFFSETIN;
  temp &= ~VC_ANA_RG_SOC_FNPLL_OFFSETIN_Msk;
  VC_ANA->RG_SOC_FNPLL_OFFSETIN = temp;

  temp = VC_ANA->RG_SOC_FNPLL_CTRL;
  temp &= ~VC_ANA_RG_SOC_FNPLL_DDSM_TEST_Msk;
  temp &= ~VC_ANA_RG_SOC_FNPLL_DDSM_EN_Msk;
  temp |= (1UL << VC_ANA_RG_SOC_FNPLL_DDSM_EN_Pos);
  VC_ANA->RG_SOC_FNPLL_CTRL = temp;
#endif
}
...
void vcana_set_gppll_150mhz(void)
{
  ...
  temp &= ~VC_ANA_RG_GPPLL_DDSM_EN_Msk;
  temp |= (0UL << VC_ANA_RG_GPPLL_DDSM_IN_Fraction_Pos);
  temp |= (24UL << VC_ANA_RG_GPPLL_DDSM_IN_Integer_Pos);
  temp |= (1UL << VC_ANA_RG_GPPLL_DDSM_EN_Pos);
  VC_ANA->RG_GPPLL_CTRL1 = temp;
...
#if defined(DEVICE_MCU_VC6330)
  temp &= ~VC_ANA_ADCCTRL_ADC_DELAY_Msk;
  temp &= ~VC_ANA_ADCCTRL_ADC_ALIEN_Msk;
#else
